...
| 3 | الرفع + التخزين + Pipeline الاستخراج (Claude vision / mock) | 🟡 جوهرها مكتمل — يتبقّى تحويل PDF→صور للمزوّد الحقيقي + طابور BullMQ |
| 4 | التقييم + النواقص والمخاطر + سجل القرارات | ✅ مكتملة |
| 5 | مكتبة وثائق الشركة + التنبيهات | 🟡 عرض المكتبة + تنبيه الانتهاء جاهز — يتبقّى الرفع وإشعارات فعلية |
| 6 | توليد العرض الفني والمالي (Word/PDF) | ⬜ قادمة (النماذج جاهزة في DB) |
| 7 | الفرق والمهام والتعليقات | 🟡 صفحة الأدمن (الأعضاء: دعوة/تغيير دور/إزالة + إعدادات مساحة العمل) + صفحة المستخدم (الحساب + كلمة المرور) جاهزة — يتبقّى المهام والتعليقات |
| 8 | الاشتراكات + حدود الاستخدام + الفوترة (الدفع مؤجَّل) | ⬜ قادمة (النماذج + الباقات جاهزة) |
| 9 | التحليلات + الأرشفة + الاختبارات + تجهيز النشر | 🟡 اختبارات الوحدات للطبقات الحرجة جاهزة — يتبقّى E2E والتحليلات |
