...
**التحقق الحالي:** `pnpm test` → ٣٩ اختبار ناجح · `pnpm --filter web typecheck` → نظيف · `pnpm --filter web build` → ناجح (٢٠ مسار). الافتراضات: `AI_PROVIDER=mock` + `STORAGE_DRIVER=disk` (لا حاجة لأي خدمة سحابية).
حساب الدخول التجريبي بعد `pnpm db:seed`: `demo@bid-os.sa` / `Demo1234!`.

**ما بعد المراحل (تحسينات إنتاجية):**
- **التخزين:** محوّل R2/S3 خاص + تنزيل بروابط موقّعة محمية بالعزل (`/api/files/download`).
- **تحليل Claude:** إدخال PDF مباشر (يقرأ الكراسات الممسوحة والرقمية باقتباسات صفحات) للباقات المدفوعة؛ معاينة مجانية = أول ٢٥ صفحة؛ حدّ صفحات صارم. تعليمات النظام مشدّدة للسياق السعودي ومنع الهلوسة.
- **مكافحة الإساءة:** توثيق جوال OTP (SMS مجرّد، console افتراضياً) — التحليل المجاني مقفل حتى التوثيق.
- **الأمان:** ترويسات أمان (`next.config`)، عزل مستأجرين صارم، روابط موقّعة، PDPL.
- **UX:** `loading.tsx` (skeletons فورية بين الصفحات)، `not-found.tsx`، `(app)/error.tsx`.
- **الأسعار:** أساسي 149 / احترافي 499 ر.س قبل الضريبة؛ فاتورة ZATCA تضيف 15%.
- **التوثيق:** `docs/DEPLOY.md` لمتغيّرات Vercel.
